...
public class BookProvider extends ContentProvider
{
    ...
    private SQLiteQueryBuilder bookFull;

//    static{
//        bookFull = new SQLiteQueryBuilder();
//        bookFull.setTables(
//                AlexandriaContract.BookEntry.TABLE_NAME + " LEFT OUTER JOIN " +
//                AlexandriaContract.AuthorEntry.TABLE_NAME + " USING (" +AlexandriaContract
// .BookEntry._ID + ")" +
//                " LEFT OUTER JOIN " +  AlexandriaContract.CategoryEntry.TABLE_NAME + " USING ("
            ...
            switch (uriMatcher.match(uri))
                {
                    ...
                    case BOOK_ID:
                        retCursor = dbHelper.getReadableDatabase().query(
                                AlexandriaContract.BookEntry.TABLE_NAME,
                                projection,
                                AlexandriaContract.BookEntry._ID + " = '" + ContentUris.parseId
                                        (uri) + "'",
                                selectionArgs,
                                null,
                                null,
                    ...
                    case AUTHOR_ID:
                        retCursor = dbHelper.getReadableDatabase().query(
                                AlexandriaContract.AuthorEntry.TABLE_NAME,
                                projection,
                                AlexandriaContract.AuthorEntry._ID + " = '" + ContentUris.parseId
                                        (uri) + "'",
                                selectionArgs,
                                null,
                                null,
                    ...
                    case CATEGORY_ID:
                        retCursor = dbHelper.getReadableDatabase().query(
                                AlexandriaContract.CategoryEntry.TABLE_NAME,
                                projection,
                                AlexandriaContract.CategoryEntry._ID + " = '" + ContentUris
                                        .parseId(uri) + "'",
                                selectionArgs,
                                null,
                                null,
                    ...
                    case BOOK_FULLDETAIL:
                        String[] bfd_projection = {
                                ...
                                "group_concat(DISTINCT " + AlexandriaContract.AuthorEntry
                                        .TABLE_NAME + "." + AlexandriaContract.AuthorEntry.AUTHOR
                                        + ") as " + AlexandriaContract.AuthorEntry.AUTHOR,
                                "group_concat(DISTINCT " + AlexandriaContract.CategoryEntry
                        ...
                        retCursor = bookFull.query(dbHelper.getReadableDatabase(),
                                bfd_projection,
                                AlexandriaContract.BookEntry.TABLE_NAME + "." +
                                        AlexandriaContract.BookEntry._ID + " = '" + ContentUris
                                        .parseId(uri) + "'",
                                selectionArgs,
                                AlexandriaContract.BookEntry.TABLE_NAME + "." +
                                        AlexandriaContract.BookEntry._ID,
            ...
            switch (match)
                {
                    case BOOK:
                    {
                        ...
                        getContext().getContentResolver().notifyChange(AlexandriaContract
                                .BookEntry.buildFullBookUri(_id), null);
                        break;
                    }
                    case AUTHOR:
                    {
                        long _id = db.insert(AlexandriaContract.AuthorEntry.TABLE_NAME, null,
                                values);
                        if (_id > 0)
                            returnUri = AlexandriaContract.AuthorEntry.buildAuthorUri(values
                                    .getAsLong("_id"));
                        else
                    ...
                    case CATEGORY:
                    {
                        long _id = db.insert(AlexandriaContract.CategoryEntry.TABLE_NAME, null,
                                values);
                        if (_id > 0)
            ...
            switch (match)
                {
                    ...
                    case AUTHOR:
                        rowsDeleted = db.delete(
                                AlexandriaContract.AuthorEntry.TABLE_NAME, selection,
                                selectionArgs);
                        break;
                    case CATEGORY:
                        rowsDeleted = db.delete(
                                AlexandriaContract.CategoryEntry.TABLE_NAME, selection,
                                selectionArgs);
                        break;
                    case BOOK_ID:
                        rowsDeleted = db.delete(
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs)
        {
            ...
            switch (match)
                {
                    case BOOK:
                        rowsUpdated = db.update(AlexandriaContract.BookEntry.TABLE_NAME, values,
                                selection,
                                selectionArgs);
                        break;
                    case AUTHOR:
                        rowsUpdated = db.update(AlexandriaContract.AuthorEntry.TABLE_NAME,
                                values, selection,
                                selectionArgs);
                        break;
                    case CATEGORY:
                        rowsUpdated = db.update(AlexandriaContract.CategoryEntry.TABLE_NAME,
                                values, selection,
                                selectionArgs);
                        break;
